...
void read_data(data_in in[node][input_block],data_t out[node][ceil(feature/32)])
{
	...
	for (int j = 0; j < node; j++)
	{
		...
		{
#pragma HLS UNROLL
			for (int i = 0; i < ceil(input_size/32); i++)
			{
#pragma HLS UNROLL
				for (int k = 0; k < 32; k++)
				{
#pragma HLS UNROLL
					if (m*input_size+i*32+k<feature)
				    {
			    ...
			    if (m*ceil(input_size/32)+i< feature)
			    {
			    	out[j][m*ceil(input_size/32)+i]=temp;
			    }
			}
		}
	}
...
void Extraction(data_t in[node][ceil(feature/32)], data_t weight[feature][block][cycle],data_tt beta[node],data_tt alpha[block][cycle],data_tt bias[block][cycle],hls::stream<data_tt> inStream[block])
{
...
void cc(data_t weight_in[feature][hidden],data_tt alpha_in[hidden],data_tt bias_in[hidden],data_t weight_out[feature][block][cycle],data_tt alpha_out[block][cycle],data_tt bias_out[block][cycle])
{

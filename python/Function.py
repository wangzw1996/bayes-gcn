...
class BinActiveFunction1(Function):
    @staticmethod
...
BinActiveFun = BinActiveFunction1.apply

class BinActiveFunction0(Function):
    @staticmethod
    def forward(ctx, input):
        ctx.save_for_backward(input)
        # size of input: [n, in_channels]
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = grad_input * where(torch.abs(input[0]) <= 1, 1, 0)
        return grad_input
BinActiveFun0 = BinActiveFunction0.apply
...
class BinActive0(torch.nn.Module):
    def __init__(self):
        super(BinActive0, self).__init__()

    def forward(self, input):

        return BinActiveFun0(input)
